// Ported from OmniRoute upstream v3.8.48 (ccr engine — retrieve API route).
// Adaptations: none — verbatim port. requireManagementAuth, retrieveBlock, queryBlock
// and sanitizeErrorMessage all exist in the fork with identical signatures, and the
// authz classifier (src/server/authz/classify.ts) already classes any non-public
// /api/* path as MANAGEMENT, so no explicit classify.ts entry is needed.
