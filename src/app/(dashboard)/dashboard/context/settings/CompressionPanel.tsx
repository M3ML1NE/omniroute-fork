...
//
// CompressionPanel — the single-source engine-grid UI for compression. Renders the master
// on/off switch, one row per catalog engine (on/off + level + link to its detail page), the
// output-styles rows, the mcpAccessibility toggle (its own endpoint / separate store), a
// read-only derived-pipeline preview, and the general settings (auto-trigger tokens +
// preserve-system-prompt mode).
//
// Fork adaptations vs upstream:
//  - The adaptiveCompression subsystem is not ported, so the upstream "Adaptive context budget"
//    row is replaced with the static legacy line it shows when unset ("off (legacy auto-trigger)").
//  - The Ultra SLM (LLMLingua) tier block is not ported (that engine is not registered here).
//
// Engine rows use the catalog label/description (hardcoded English) directly — NOT i18n — so they
// stay deterministic. Human-facing chrome (master, general) keeps the app's i18n.

import Link from "next/link";
import { useEffect, useState } from "react";
import { useTranslations, useLocale } from "next-intl";
// Import Card/Toggle from their direct module paths rather than the @/shared/components barrel:
// the barrel transitively pulls a heavy/Node-only module that hangs the vitest/jsdom component
// test. Direct imports resolve identically under Next.js.
...
export default function CompressionPanel() {
  ...
  // Locale-gated styles (e.g. terse-cjk → zh) are only OFFERED under their locale.
  ...
  // Persist a merge-patch. The DB persists `engines` as one whole row, so callers that touch an
  // engine pass the full engines map to avoid dropping the other engines.
  ...
  return (
    <Card className="p-6" data-testid="compression-panel">
      ...
      {/* Auto-trigger status — the fork does not ship adaptiveCompression, so this row shows the
          static legacy auto-trigger note (upstream showed the same string when the budget was unset). */}
      <div
        data-testid="adaptive-target-preview"
        className="mb-4 rounded-md border border-border/60 bg-bg-subtle px-3 py-2 text-xs text-text-muted"
      >
        Adaptive context budget: off (legacy auto-trigger)
      </div>
      ...
      {/* Output Styles — response-output instruction injection (catalog-driven) */}
