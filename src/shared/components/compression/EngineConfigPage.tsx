...
// settings store. The on/off + level for ALL engines live in the settings page;
// only these have a place to persist the extra per-engine fields edited on this
// page. Structural engines (lite, headroom, session-dedup, ccr) have no
// sub-object — their page keeps the detail form + preview but has nothing extra
// to persist this phase.
...
export function EngineConfigPage({ engineId }: { engineId: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // the on/off + level live on the settings page. 404/null/missing = schema defaults.
  ...
  // level are owned by the settings page and are NOT written here.
  async function handleSave() {
    ...
    // Strip the `enabled` key — engine on/off is the settings page's responsibility.
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-3xl">
      ...
      {/* ── Settings pointer (on/off + level live there now) ── */}
