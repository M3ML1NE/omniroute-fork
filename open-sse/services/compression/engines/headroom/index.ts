// Ported from OmniRoute upstream v3.8.48 (headroom engine), adapted: python/Codex integrations removed
/**
 * headroom compression engine — SmartCrusher: tabular compaction of homogeneous JSON arrays.
 *
 * Fork note: this is the pure-TS tabular/toon/gcf compaction core only. The upstream
 * tag also shipped a python-subprocess proxy path (src/lib/headroom/*, /api/headroom/*)
 * and a Codex/quota-fetch combo-ranking integration (services/combo/headroomRanking.ts) —
 * NONE of that lives in the engine directory and NONE of it is ported here. The engine
 * itself never referenced python/Codex/quota; only the optional `@toon-format/toon`
 * dependency was dropped (toon.ts is a fail-open stub → GCF is the sole encoder).
 *
