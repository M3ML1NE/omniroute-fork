// Ported from OmniRoute upstream v3.8.48 (ccr engine).
// Adaptations:
//   - none to the algorithm; verbatim port of the in-module content-addressed store.
//   - Storage design: the tag stored originals in an in-process bounded Map keyed by
//     `${principalId} ${hash}` (NOT a src/lib/db module and NOT the key_value namespace).
//     The fork's compression.ts / readCache.ts have no compatible content-addressed
//     store, so the upstream in-module Map is ported as-is. Retrieval survives only for
//     the process lifetime (documented upstream contract: markers "may have expired").
//   - Relies on CompressionEngineApplyOptions.principalId (added in engines/types.ts).
