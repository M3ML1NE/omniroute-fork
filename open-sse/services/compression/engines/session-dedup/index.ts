// Ported from OmniRoute upstream v3.8.48 (session-dedup engine).
// Adaptations:
//   - none to the algorithm; verbatim port including the intra-message dedup fix
//     ("add intra-message dedup to session-dedup engine (#6467)", already in the tag).
//   - Fuzzy pass depends on the CCR engine's in-module store (storeBlock); CCR was
//     ported alongside this engine, so the import resolves within the fork.
//   - Uses CompressionEngineApplyOptions.principalId (added in engines/types.ts) to
//     scope fuzzy-pass CCR markers per tenant.
