...
test("getModelInfoCore resolves unique non-openai unprefixed model", async () => {
  const info = await getModelInfoCore("GigaChat-2-Max", {});
  assert.equal(info.provider, "gigachat");
  assert.equal(info.model, "GigaChat-2-Max");
});
...
test("getModelInfoCore returns explicit not-found metadata for unknown unprefixed model", async () => {
  const info = await getModelInfoCore("claude-haiku-4.5", {});
  assert.equal(info.provider, null);
  assert.equal((info as any).errorType, "model_not_found");
  assert.match((info as any).errorMessage, /Unable to determine provider/i);
});
